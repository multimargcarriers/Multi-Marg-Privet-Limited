...
{
	$sq = "INSERT INTO lr_details (awb,value,invdate,invoice,part,eway,quantity) 
       VALUES ('$awb','$value','$dt','$inv','$part','$eway','$qty')";

mysqli_query($con, $sq) or die(mysqli_error($con));
}}

$sql1 = "INSERT IGNORE INTO track
         (pid,awb,status,remarks,date,location)
         VALUES
         (0,'$awb','SHIPMENT BOOKED','PICKUP DONE','$date','$origin')";
...
if ($t == "Operation Team") {
    ...
    echo "<script>
            ...
            window.location.href = '../lr.php?code=" . urlencode($nextPid) . "';
          </script>";
} else {
    echo "<script>
            alert('DATA SAVED SUCCESSFULLY');
