...
if( !empty($awb) || !empty($date) || !empty($mode) || !empty($client) || !empty($origin)  || !empty($destination) || !empty($consignor)|| !empty($oconsignee) || !empty($tob) || !empty($insured) || !empty($description) || !empty($remarks) || !empty($clerk)|| !empty($box)|| !empty($aweight)|| !empty($cweight))
{
...
{
$sql="INSERT INTO trip (pid,awb,date,mode,client,origin,destination,consignor, consignee, type_of_delivery, insured, description,remarks,clerk_name,box,actual_wt,charge_wt) VALUES('0','$awb','$date','$mode','$client','$origin','$destination','$consignor','$consignee','$tob','$insured','$description','$remarks','$clerk','$box','$aweight','$cweight')";
#execute above sql
