...
if (!empty($fr) && !empty($to) && !empty($client)) {
    ...
    $retval = mysqli_query($con, "SELECT * FROM trip where client='$client' and date between '$fr' AND '$to' ORDER BY pid DESC");
}
elseif (!empty($fr) && !empty($to) && empty($client)) {
    
    $retval = mysqli_query($con, "SELECT * FROM trip where date between '$fr' AND '$to' ORDER BY pid DESC");
}
elseif (empty($fr) && empty($to) && !empty($client)) {
   
    $retval = mysqli_query($con, "SELECT * FROM trip where client='$client' ORDER BY pid DESC");
}
// Fetch all trip records
else
{
$retval = mysqli_query($con, "SELECT * FROM trip ORDER BY pid DESC");
}
