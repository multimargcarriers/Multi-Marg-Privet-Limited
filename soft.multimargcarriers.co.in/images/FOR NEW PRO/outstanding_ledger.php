		...
		<div class="page-breadcrumb">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                        ...
                        <div class="body">
                            <h2 class="card-inside-title">Outstanding Ledger</h2>
...
<div class='company'>
    ...
    <tbody>  
    <?php
   $retval = mysqli_query($con, "SELECT * FROM bills WHERE client='$client' GROUP BY invoice HAVING COUNT(invoice)>=1 ORDER BY invoice DESC LIMIT $start, $limit") or die(mysqli_error($con));
    if(! $retval ) {
...
while($row=mysqli_fetch_array($retval)) {
    $consig=$row['invoice'];
       $date1=$row['invoice_date'];
         $gstin=$row['gst'];
   $date="2025-09-25";
  echo "<tr><td>".$row['invoice']."</td><td>".date("d-m-Y", strtotime($row['invoice_date']))."</td> " ; 
...
?>
  ...
  <table id="salesTable">
    ...
    <tbody>  
  <?php  $sql1 = "SELECT * FROM outstanding WHERE client='$client' LIMIT $start1, $limit1";
$result1 = mysqli_query($con, $sql1) or die(mysqli_error($con));
$ht=0;
while ($row1 = mysqli_fetch_assoc($result1)) {
    $amt = $row1['amount'];
    $ht+=$amt;
    echo "<tr><td>".date("d-m-Y", strtotime($row1['date']))."</td><td>NEFT</td><td>".$amt."</td></tr>" ; 
}
?>
