...
const UpdateBill = () => {
  ...
  const [form, setForm] = useState({ billNo: "", client: "", amount: "", status: "", remarks: "" });
  ...
  const handleSelect = (bill) => {
    ...
    setForm({ billNo: bill.billNo || "", client: bill.client || "", amount: bill.amount || bill.total || "", status: bill.status || "pending", remarks: bill.remarks || "" });
  };
  ...
  return (
    <div>
      <div style={{ marginBottom: "2rem" }}>
        <h3 style={{ fontSize: "1.8rem", marginBottom: "0.25rem" }}>Update Bill</h3>
        <p className="text-muted">Search and update existing bills.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: selectedBill ? "1fr 1fr" : "1fr", gap: "2rem" }}>
            ...
            <tbody>
              {filtered.map((item, index) => (
                <tr key={index} style={{ borderTop: "1px solid rgba(0, 0, 0, 0.05)", cursor: "pointer", background: selectedBill?.id === item.id ? "rgba(13, 110, 253, 0.05)" : "transparent" }}
                  ...
                  <td style={{ padding: "1rem" }}><span style={{ display: "inline-flex", alignItems: "center", whiteSpace: "nowrap" }}><RupeeIcon size={14} />&nbsp;{parseFloat(item.amount || item.total || 0).toFixed(2)}</span></td>
        ...
        {selectedBill && (
          <div className="glass-panel" style={{ padding: "1.5rem" }}>
            <h4 style={{ marginTop: 0 }}>Edit Bill</h4>
            <form onSubmit={handleUpdate}>
              ...
              <div className="form-group">
                ...
                <textarea className="form-control" style={{ height: 80, padding: "0.8rem 1.2rem", resize: "vertical" }} value={form.remarks} onChange={(e) => setForm({ ...form, remarks: e.target.value })} />
              </div>
              <button type="submit" className="btn btn-primary" style={{ width: "100%", height: 50 }}>
                <FileText size={18} /> Update Bill
              </button>
            </form>
          </div>
        )}
