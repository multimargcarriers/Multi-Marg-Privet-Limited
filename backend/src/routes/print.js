...
<table>
  ...
  <tr style="font-weight:bold;"><td>Total</td><td>${(
    parseFloat(
      booking.freight_charge || booking.frieght || booking.freight || 0,
    ) +
    parseFloat(booking.awb_charge || 0) +
    parseFloat(booking.pickup_charge || 0) +
    parseFloat(booking.delivery_charge || 0) +
    parseFloat(booking.packaging_charge || 0) +
    parseFloat(booking.handling_charge || 0)
  ).toFixed(2)}</td></tr>
</table>
<div class="footer">
  ...
  <div><div class="line">Authorised Signatory</div></div>
    ...
    res.json({ success: true, data: { html, lrNumber, booking } });
  }),
);
...
<div class="footer" style="display:flex; justify-content:space-between;">
  <div><div class="line">Authorised Signatory</div></div>
    ...
    res.json({ success: true, data: { html, billNo, bill } });
  }),
);
...
<div style="margin-top:30px; display:flex; justify-content:space-between;">
  ...
  <div style="text-align:center; width:30%; border-top:1px solid #000; padding-top:5px; font-size:11px;">Authorised Signatory</div>
    ...
    res.json({ success: true, data: { html, tripNo, trip } });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const { trip: tripNo, client } = req.params;
...
<div style="margin-top:30px; display:flex; justify-content:flex-end;">
  <div style="text-align:center; width:30%; border-top:1px solid #000; padding-top:5px; font-size:11px;">Authorised Signatory</div>
    ...
    res.json({ success: true, data: { html, tripNo, client } });
  }),
);
