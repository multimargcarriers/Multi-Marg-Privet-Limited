...
exports.getRoot_1 = async (req, res) => {
  ...
  const data = await getOrSet(`${CACHE_KEY}_${client || "all"}_${from || "all"}_${to || "all"}`, async () => {
    // Real Firebase logic
    ...
    return {
      summary: {
        ...
        totalFreight: filtered.reduce((s, b) => s + parseFloat(b.freight || b.frieght || 0), 0),
        totalBoxes: filtered.reduce((s, b) => s + parseInt(b.box || 0), 0),
        totalWeight: filtered.reduce((s, b) => s + parseFloat(b.charge_wt || b.weight || 0), 0)
      },
