...
exports.getRoot_1 = async (req, res) => {
  ...
  let query = db.collection("vendor_mis").orderBy("createdAt", "desc").limit(100);
  
  // If user is a Vendor, they can only see their own entries
  if (user && user.role === 'Vendor') {
    query = db.collection("vendor_mis").where("createdBy", "==", user.id).orderBy("createdAt", "desc").limit(100);
  }
